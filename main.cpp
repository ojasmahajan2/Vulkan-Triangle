...
//=====================================================================================================================
    ...
    PolygonMode RasterizationMode = PolygonMode::GEOMETRY_FILL;
    ...
    void mainLoop() {
		...
		while (running) {
			while (SDL_PollEvent(&event)) {
				if (event.type == SDL_EVENT_QUIT || state[SDL_SCANCODE_ESCAPE]) {
					running = false;
				} 
...
//=====================================================================================================================
    ...
    void drawFrame() {
		...
		switch (RasterizationMode) {
			case PolygonMode::GEOMETRY_FILL:
				activePipeline = *graphicsPipeline;
				break;
			case PolygonMode::WIREFRAME:
				activePipeline = *wireframePipeline;
				break;
			case PolygonMode::VERTEX_POINT:
				activePipeline = *pointPipeline;
				break;
		}
		commandBuffers[currentFrame].bindPipeline(vk::PipelineBindPoint::eGraphics, activePipeline);
		...
		pushData.mvp = glm::rotate(glm::mat4(1.0f), time * glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f));

		commandBuffers[currentFrame].pushConstants<pushConstantData>(
			*pipelineLayout, vk::ShaderStageFlagBits::eVertex, 0, pushData
		);
	...
	void inputWindow(SDL_Event& event, const bool*& state) {
		if (state[SDL_SCANCODE_1]) {
			...
			RasterizationMode = PolygonMode::VERTEX_POINT;
		}
		else if (state[SDL_SCANCODE_2]) {
			backgroundColor = { 0.01f, 0.01f, 0.01f, 1.0f };
			RasterizationMode = PolygonMode::WIREFRAME;
		}
		else if (state[SDL_SCANCODE_3]) {
			backgroundColor = { 0.01f, 0.01f, 0.02f, 1.0f };
			RasterizationMode = PolygonMode::GEOMETRY_FILL;
		}
	}
